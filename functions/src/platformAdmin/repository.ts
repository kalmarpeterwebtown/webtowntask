...
async function listCollectionPaths(collectionRef: CollectionReference<DocumentData>) {
  const snap = await collectionRef.get()
  return snap.docs.map((entry) => entry.ref.path)
}

async function getExistingDocPath(...segments: string[]) {
...
async function loadOrganizationRegisteredUsers(orgId: string, memberUserIds: string[]) {
  ...
  return users.filter((user: PlatformRegisteredUser | null): user is PlatformRegisteredUser => user !== null)
}

async function collectOrganizationDeletionPlan(orgId: string, deleteRegisteredUsers: boolean) {
  ...
  const sharedUserCleanups: Array<{ userId: string; nextCurrentOrgId: string | null; notificationPaths: string[] }> = []
...
async function deleteDocumentPaths(paths: string[]) {
...
export function createPlatformAdminRepository() {
  const findUserFootprintByEmail = async (email: string): Promise<PlatformUserFootprint> => {
      const warnings: string[] = []
      const hits: PlatformDocHit[] = []
  ...
  const repository = {
    ...
    async listAuditLogs(limit: number): Promise<PlatformAuditLogEntry[]> {
      ...
      return snap.docs.map((entry) => {
        ...
        } satisfies PlatformAuditLogEntry
      })
    },

    async detachUserFromOrganization(email: string, orgId: string) {
      const footprint = await findUserFootprintByEmail(email)
      const deletableHits = footprint.hits.filter((hit: PlatformDocHit) =>
        hit.orgId === orgId && ['orgMember', 'projectMembership', 'teamMembership', 'invitation', 'orgMembership', 'notification'].includes(hit.kind))
    ...
    async hardDeleteUserFootprint(email: string): Promise<DeleteUserFootprintResult> {
      const footprint = await findUserFootprintByEmail(email)
      ...
      const otherPaths = footprint.hits
        .filter((hit: PlatformDocHit) => !(footprint.userId && hit.path.startsWith(docPath('users', footprint.userId))))
        .map((hit: PlatformDocHit) => hit.path)
    ...
    async deleteAuthUser(email: string): Promise<DeleteAuthUserResult> {
      try {
    ...
    async previewOrganizationDeletion(orgId: string): Promise<PlatformOrganizationDeletionPreview> {
      ...
      return {
        ...
        deletableUserCount: plan.registeredUsers.filter((user: PlatformRegisteredUser) => user.canDeleteProfile).length,
        sharedUserCount: plan.registeredUsers.filter((user: PlatformRegisteredUser) => !user.canDeleteProfile).length,
        estimatedDeleteCount: plan.estimatedDeleteCount,
      }
    },

    async deleteOrganization(orgId: string, deleteRegisteredUsers: boolean): Promise<DeleteOrganizationResult> {
    ...
    async writeAuditLog(entry: PlatformAuditLogEntry) {
