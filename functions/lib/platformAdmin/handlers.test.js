...
import { createPlatformAdminHandlers } from './handlers.js';
...
function createCallableRequest(data, auth = superAdminAuth) {
    return {
        data,
        auth,
        rawRequest: {},
...
describe('platform admin handlers', () => {
    ...
    it('rejects footprint reads for non-super-admin users', async () => {
        await expect(handlers.findUserFootprint(createCallableRequest({ email: 'dev@webtown.hu' }, {
            uid: 'user-1',
